...
/* -------- Number Mode left -------- */
bool numl_mode_process(uint16_t keycode, keyrecord_t *record) {
    ...
    switch (keycode) {
        ...
        default:
            ...
            if (!record->event.pressed) {
                return smart_layer_disable(_NUML);
            }
            break;
    }
    return true;
...
/* -------- Number Mode right -------- */
bool numr_mode_process(uint16_t keycode, keyrecord_t *record) {
    // Assess if we should exit layermode or continue processing normally.
    switch (keycode) {
        case OS_LSFT ... OSR_SFT:
        case KC_1 ... KC_0:
        ...
        default:
            // All other keys disable the layer mode on keyup.
            if (!record->event.pressed) {
                return smart_layer_disable(_NUMR);
            }
            break;
    }
    return true;
...
bool fun_mode_process(uint16_t keycode, keyrecord_t *record) {
    switch (keycode) {
        case KC_F3:
        case F5_TH:
        case KC_F8:
        ...
        default:
            if (!record->event.pressed) {
                return smart_layer_disable(_FUN);
            }
            break;
    }
    return true;
