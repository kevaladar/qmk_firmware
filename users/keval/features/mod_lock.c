...
uint8_t        NUML_MODLOCK_STATES = sizeof(modlock_states) / sizeof(mod_lock_state_t);
uint8_t        NUMR_MODLOCK_STATES = sizeof(modlock_states) / sizeof(mod_lock_state_t);

void process_mod_lock(uint16_t keycode, keyrecord_t *record) {
    mod_lock_state_t *curr_state = NULL;

    for (int i = 0; i < NUML_MODLOCK_STATES; ++i) {
