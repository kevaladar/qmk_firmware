...
// comment out _DF_MAC and _ALT_MAC
/**** Processin' Records ****/
...
bool process_record_user(uint16_t keycode, keyrecord_t *record) {
    ...
    if (_numl_mode_active) {
        _numl_mode_active = num_mode_process(keycode, record);
    } else if (_fun_mode_active) {
        _fun_mode_active = func_mode_process(keycode, record);
        ...
        _macro_mode_active = smart_oneshot_process(keycode, record, _FUN);
    }
    ...
    switch (keycode) {
        ...
        case SYMMODE: 
            ...
            return false;
        // case MCRMODE: 
        //     _macro_mode_active = smart_layer_enable(_MERG);
        //     return false;
