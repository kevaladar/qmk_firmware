...
// Smart Layer config
bool _numl_mode_active = false;
bool _numr_mode_active = false;
bool _fun_mode_active = false;
...
bool process_record_user(uint16_t keycode, keyrecord_t *record) {
    ...
    if (_numl_mode_active) {
        _numl_mode_active = numl_mode_process(keycode, record);
    } else if (_numr_mode_active) {
        _numr_mode_active = numr_mode_process(keycode, record);
    } else if (_fun_mode_active) {
        _fun_mode_active = fun_mode_process(keycode, record);
    ...
    switch (keycode) {
        ...
        // Layer Modes
        case NUMLMODE: 
            _numl_mode_active = smart_layer_enable(_NUML);
            return false;
        case NUMRMODE: 
            _numr_mode_active = smart_layer_enable(_NUMR);
            return false;            
        case FUNMODE: 
            _fun_mode_active = smart_layer_enable(_FUN);
            return false;
        case NAVMODE: 
...
/*
        case ISLINUX: {
            if(record->event.pressed){
                ...
                df_layer = is_linux ? _DHORF : _DF_MAC;
                layer_move(df_layer);
...
uint8_t        NUML_NSHOT_STATES = sizeof(nshot_states) / sizeof(nshot_state_t);
uint8_t        NUMR_NSHOT_STATES = sizeof(nshot_states) / sizeof(nshot_state_t);
