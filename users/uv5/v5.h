...
// // Primary Thumb Keys
// #define THM_LH0     MO(_NAV)
// #define THM_LH1     QK_REP

// #define THM_RH0     MO(_SYM)
...
// // place holder alt can be whatever, night would be good choice
// #define LAYER_ALT \
...
// // just home row mod here
// /* Wrapper layout giúp bạn viết layout 34 phím mà không cần nhớ tên macro dài ngoằng
// LAYOUT_split_3x5_2 là layout mặc định của bàn phím split kiểu 3 hàng mỗi bên, 2 phím cái (thumbs)
// Dùng LAYOUT_34key_w(...) là auto wrap lại thành layout đúng chuẩn
// */
//  #define LAYOUT_34key_w(...) LAYOUT_split_3x5_2(__VA_ARGS__)

// // Macros to add inverted-T GACS mods
// /* #define URM_L(k1) LGUI_T(k1)   // URM_L = Home row trái, gán GUI khi giữ
//  #define URM_R(k1) RGUI_T(k1)   // URM_R = Home row phải, gán GUI khi giữ
//  */
// #define HRM_L(k0, k1, k2, k3) LGUI_T(k0),LALT_T(k1),LCTL_T(k2),LSFT_T(k3)  // HRM_L: Home Row Mods bên trái - phím đầu (k0) để nguyên   - phím 2: giữ = Alt, tap = ký tự - phím 3: giữ = Ctrl - phím 4: giữ = Shift
// #define HRM_R(k0, k1, k2, k3) RSFT_T(k0),RCTL_T(k1),LALT_T(k2), RGUI_T(k3)  // HRM_R: Home Row Mods bên phải, tương tự nhưng đảo chiều

// /*
//  // Macros to do the same but for mac
//  // Macros tương tự cho macOS (dùng Ctrl thay GUI)
...
// #define HRM(k) HR_MODTAP(k) // Gọi HRM(...) sẽ thực thi HR_MODTAP(...)
// #define HR_MODTAP( \
...
//     Bạn truyền vào 30 phím + 4 phím cái (l16, l17, r16, r17)

//     Macro sẽ:

//         Gán GUI cho 1 vài phím hàng đầu

//         Gán Alt, Ctrl, Shift cho hàng giữa (home row)

//         Các phím khác để nguyên

//     Kết quả: bạn chỉ cần nhập keycodes đơn giản, macro này tự động gắn mod-tap theo logic sẵn.
// */
// /*
...
// */
// /*Phiên bản cho macOS
//  Tương tự HR_MODTAP, nhưng các modifier là LCTL, LGUI,... theo đúng chuẩn macOS.
// */

// /*Tóm gọn cho dễ nhớ:
// Macro	Mục đích
// LAYOUT_34key_w	Gọn hơn cho LAYOUT_split_3x5_2
// URM_L, URM_R	Mod-tap GUI cho phím home row
// HRM_L, HRM_R	Gán mod Alt/Ctrl/Shift cho 4 phím home row
// MAC_URM_*	Phiên bản tương tự cho macOS (Ctrl thay GUI)
// HR_MODTAP	Gọi 1 lần, auto sắp xếp modifier cho nguyên layout
// MAC_HR_MODTAP	Phiên bản macOS của HR_MODTAP
// */